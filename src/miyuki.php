...
class Miyuki
{
    ...
    public function write($path)
    {
        if($path === false || $path === null)
            $finalPath = tempnam(sys_get_temp_dir(), '') . '.' . $this->imageType['extension'];
        elseif($path === true)
            $finalPath = $this->imagePath;
        else
            $finalPath = $path;

        $this->imageBlock -> writeImage($finalPath);
    ...
    function setImageCompression($switch)
    {
        $this->imageBlock -> setImageCompression($switch);
